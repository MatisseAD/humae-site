...
export async function getNews(): Promise<NewsItem[]> {
  try {
    const { data, error } = await getSupabase().from('news').select('*').order('id')
...
export async function getNewsItem(id: string): Promise<NewsItem | null> {
  const { data, error } = await getSupabase().from('news').select('*').eq('id', id).single()
  if (error && error.code !== 'PGRST116') throw new Error(error.message)
  return data as NewsItem | null
}

export async function addNews(item: Omit<NewsItem, 'id'>) {
  const { data, error } = await getSupabase().from('news').insert(item).select().single()
  if (error) throw new Error(error.message)
  return data as NewsItem
}

export async function updateNews(id: string, updates: Partial<Omit<NewsItem, 'id'>>) {
  const { data, error } = await getSupabase().from('news').update(updates).eq('id', id).select().single()
  if (error) throw new Error(error.message)
  return data as NewsItem
}
